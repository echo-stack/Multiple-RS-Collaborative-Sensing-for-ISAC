 function rmse_V=joint_test_v(SNR1,f_c1,Nsymbols1,Ncarriers1,delta_f1)
%% parameter setting
SNR=SNR1;
f_c = f_c1; % carrier frequency
Nsymbols = Nsymbols1; %number of OFDM symbols
Ncarriers =Ncarriers1*2;  %number of subcarriers
delta_f =delta_f1;  % subcarrier spacing

% SNR=15;
% f_c = 24*10^9; % carrier frequency
% Nsymbols = 140; %number of OFDM symbols
...
CSIRS_carriers=Ncarriers;%DMRS子载波数
alpha=1;%DMRS映射缩放因子
%% CSIRS频域映射
...
%                     sum_R=sum_R+(M_R-R(1))^2;
                    ...
                    % 根据论文中的公式可以求出速度
                    M_v = ((index_v-1)*c0 / (2*f_c*Nsymbols*Tofdm));
                    %测速
                    figure(3)
                    div_FFT = div_FFT / (Ncarriers/K_comb);
                    plot(abs(div_FFT/max_v));  
%                     text(index_v,abs(div_FFT(floor(index_v))),'o','color','r');  %标记出峰值点
%                     text(floor(index_v)+1,abs(div_FFT(round(index_v))),['(',num2str(floor(index_v)),',',num2str(abs(div_FFT(round(index_v)))),')'],'color','k');  %标记出峰值点
                    xlabel('index_v');
                    ylabel('Mag');
