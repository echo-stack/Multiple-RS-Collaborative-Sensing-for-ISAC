...
for m=1:Nsymbols*(4/14)
    ...
    for k=1:PRS_carriers
        r2(m,k)=1/sqrt(2)*(1-2*c2(2*k))+1j*1/sqrt(2)*(1-2*c2(2*k+1)); %产生待传输的DMRS序列
    end
end
r1=r1.';
r2=r2.';
    ...
    switch process
        case 1% ==========2D FFT====================
            ...
            for k=1:Nsymbols*(4/14)
                ...
                x=14*floor((k-1)/4)+2*(k-4*floor((k-1)/4))+1+(-1)^j+1;%1，2，3，4，5，6对应3，5，9，11，17，19 ……
            ...
            %% 提取用于测速的导频信息矩阵
            %                     symbols=zeros(Ncarriers,Nsymbols/K_comb);%存放承载PRS的子载波序号
            %                     for m=1:Ncarriers
            %                        for n=1:Nsymbols/K_comb
            %                            symbols(m,n)=K_comb*(n-1)+(1+(-1)^mod(m,2))/2+1;%计算OFDM符号的序号
            %                        end
            %                     end
...
%             plot(abs(div_IFFT));
%             text(index_r,abs(div_IFFT(floor(index_r))),'o','color','r');  %标记出峰值点
%             text(floor(index_r)+20,abs(div_IFFT(round(index_r))),['(',num2str(floor(index_r)),',',num2str(abs(div_IFFT(round(index_r)))),')'],'color','k');  %标记出峰值点
            xlabel('Distance/m');
            ylabel('Mag');
            ...
            %% ======================测速===================================
            %                     index_v=0;%测速的索引
            %                     div_FFT=zeros(1,Nsymbols/K_comb);%做FFT寻找峰值时，储存FFT的结果
            %                     for i=1:Ncarriers
